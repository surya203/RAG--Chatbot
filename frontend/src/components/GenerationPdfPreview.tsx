...
import { fetchGenerationSourceBlobUrl } from "@/services/exam";

interface GenerationPdfPreviewProps {
  sourceId: string;
  title: string;
  onClose: () => void;
}

export default function GenerationPdfPreview({
  sourceId,
  title,
  onClose,
}: GenerationPdfPreviewProps) {
  ...
  useEffect(() => {
    ...
    fetchGenerationSourceBlobUrl(sourceId)
      .then((url) => {
        if (revoked) {
  ...
  const displayName = title.toLowerCase().endsWith(".pdf") ? title : `${title}.pdf`;
